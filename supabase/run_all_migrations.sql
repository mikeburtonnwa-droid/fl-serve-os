-- =============================================================================
-- SERVE OS - Complete Migrations Script
...
VALUES
  ('AI Readiness Assessment', 'Comprehensive assessment template for evaluating an organization''s readiness for AI implementation.', 'assessment', '# AI Readiness Assessment\n\n## Executive Summary\n\n[Organization Name] is currently positioned at a **[Score]%** AI readiness level...', 'markdown', 'system', true, ARRAY['assessment', 'ai-readiness', 'foundation'], 'general', 'Initial client assessment', 'published'),
  ('Process Automation Opportunity Assessment', 'Template for identifying and prioritizing automation opportunities within an organization.', 'assessment', '# Process Automation Opportunity Assessment\n\n## Overview\n\nThis assessment identifies automation opportunities...', 'markdown', 'system', true, ARRAY['assessment', 'automation', 'process'], 'general', 'Process improvement', 'published'),
  ('AI Implementation Proposal', 'Standard proposal template for AI/automation implementation engagements.', 'proposal', '# AI Implementation Proposal\n\n**Prepared for:** [Client Name]\n**Prepared by:** [Consultant Name]...', 'markdown', 'system', true, ARRAY['proposal', 'ai', 'implementation'], 'general', 'Client proposals', 'published'),
  ('Monthly Engagement Status Report', 'Standard template for monthly client engagement status updates.', 'report', '# Monthly Status Report\n\n**Engagement:** [Engagement Name]\n**Client:** [Client Name]...', 'markdown', 'system', true, ARRAY['report', 'status', 'monthly'], 'general', 'Client reporting', 'published'),
  ('Implementation Project Plan', 'Detailed project plan template for AI/automation implementations.', 'plan', '# Implementation Project Plan\n\n**Project:** [Project Name]\n**Client:** [Client Name]...', 'markdown', 'system', true, ARRAY['plan', 'project', 'implementation'], 'general', 'Project planning', 'published'),
  ('Go-Live Readiness Checklist', 'Pre-deployment checklist to ensure all requirements are met before go-live.', 'checklist', '# Go-Live Readiness Checklist\n\n**Project:** [Project Name]\n**Target Go-Live Date:** [Date]...', 'markdown', 'system', true, ARRAY['checklist', 'go-live', 'deployment'], 'general', 'Deployment preparation', 'published')
...
WITH RECURSIVE lineage AS (
    SELECT e.id, e.name, c.name AS client_name, 'self'::TEXT, NULL::TIMESTAMPTZ, 0
    FROM engagements e LEFT JOIN clients c ON e.client_id = c.id
    WHERE e.id = engagement_uuid
    UNION ALL
    SELECT e.id, e.name, c.name, 'parent'::TEXT, ec.cloned_at, l.depth - 1
    FROM lineage l
    ...
    WHERE l.relationship IN ('self', 'parent') AND l.depth > -5
    UNION ALL
    SELECT e.id, e.name, c.name, 'child'::TEXT, ec.cloned_at, l.depth + 1
    FROM lineage l
    JOIN engagement_clones ec ON ec.source_engagement_id = l.engagement_id
    JOIN engagements e ON e.id = ec.target_engagement_id
    LEFT JOIN clients c ON e.client_id = c.id
    WHERE l.relationship IN ('self', 'child') AND l.depth < 5
)
